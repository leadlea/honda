...
class VeteranProfile:
    ...
    user_id: str
    business_title: str = ""
    skills: List[Dict] = field(default_factory=list)  # [{"name": str, "level": str, "years": int, "certifications": List[str]}]
    experiences: List[Dict] = field(default_factory=list)  # [{"title": str, "department": str, "duration": int, "achievements": List[str]}]
    preferences: Dict = field(default_factory=dict)  # {"preferred_roles": List[str], "work_style": str, "locations": List[str]}
    privacy_settings: Dict = field(default_factory=dict)  # {"is_publicly_visible": bool, "external_contact": bool}
    ...
    def from_dynamodb_item(cls, item: Dict) -> 'VeteranProfile':
        ...
        return cls(
            ...
            questionnaire_responses=json.loads(item.get('questionnaire_responses', '[]')),
        ...
        if self.privacy_settings:
            if 'is_publicly_visible' in self.privacy_settings:
                if not isinstance(self.privacy_settings['is_publicly_visible'], bool):
                    errors.append("privacy_settings.is_publicly_visible must be boolean")
    ...
    def update_privacy_settings(self, settings: Dict) -> None:
        ...
        if 'is_publicly_visible' in settings:
            self.is_publicly_visible = "true" if settings['is_publicly_visible'] else "false"
