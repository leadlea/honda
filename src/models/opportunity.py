...
class Opportunity:
    ...
    location: str = ""
    type: str = (
        ""  # 'internal_transfer', 'external_position', 'consulting', 'project_based'
    )
    source: str = ""  # 'internal', 'external'
    company: str = ""
    salary_range: Dict = field(
        default_factory=dict
    )  # {"min": int, "max": int, "currency": str}
    is_active: bool = True
    ...
    def validate(self) -> List[str]:
        ...
        if self.salary_range:
            if "min" in self.salary_range and not isinstance(
                self.salary_range["min"], (int, float)
            ):
                errors.append("salary_range.min must be a number")
            if "max" in self.salary_range and not isinstance(
                self.salary_range["max"], (int, float)
            ):
            ...
            if "min" in self.salary_range and "max" in self.salary_range:
                if self.salary_range["min"] > self.salary_range["max"]:
                    errors.append(
                        "salary_range.min cannot be greater than salary_range.max"
                    )
