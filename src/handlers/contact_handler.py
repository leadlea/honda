...
from src.repositories.public_profile_repository import ContactRequestRepository, PublicProfileRepository
from src.repositories.user_repository import UserRepository
from src.models.public_profile import ContactRequest
from src.services.bedrock_client import BedrockClient
from src.utils.security_audit import audit_security_event, SecurityEventType, RiskLevel
...
class ContactHandler:
    ...
    def submit_contact_request(self, event: Dict, context: Any) -> Dict:
        ...
        try:
            ...
            required_fields = ['requester_name', 'requester_email', 'requester_company', 'message']
    ...
    def _is_rate_limited(self, requester_email: str, profile_id: str) -> bool:
        ...
        try:
            ...
            email_requests = [
                ...
                if req.requester_email == requester_email and req.profile_id == profile_id
            ]
...
Score: """
    ...
    def _simple_spam_detection(self, request_data: Dict) -> float:
        ...
        spam_keywords = [
            'guaranteed income', 'work from home', 'no experience required',
            'click here', 'limited time', 'act now', 'make money fast',
            'free money', 'earn $', 'investment opportunity'
        ]
        
        message = request_data.get('message', '').lower()
        company = request_data.get('requester_company', '').lower()
        
        spam_count = sum(1 for keyword in spam_keywords if keyword in message or keyword in company)
    ...
    def _notify_veteran_of_contact_request(self, profile: Dict, contact_request: ContactRequest):
        ...
        try:
            ...
            logger.info(f"Notification: New contact request for profile {profile['profile_id']} "
    ...
    def _facilitate_contact(self, contact_request: ContactRequest, veteran_user: Dict, notes: str):
        ...
        try:
            ...
            logger.info(f"Facilitating contact between {contact_request.requester_email} "
