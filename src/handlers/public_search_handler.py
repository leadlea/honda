...
class PublicSearchHandler:
    ...
    def search_veterans(self, event: Dict, context: Any) -> Dict:
        ...
        try:
            ...
            limit = min(int(query_params.get('limit', 20)), 50)  # Max 50 results per page
            
            paginated_profiles = self._paginate_results(ranked_profiles, page, limit)
            
            # Format response
            formatted_profiles = [
                self._format_public_profile(profile) 
    ...
    def _rank_profiles_with_ai(self, profiles: List[Dict], search_query: str) -> List[Dict]:
        ...
        try:
            ...
            for profile in profiles:
                summary = {
                    ...
                    'departments': [exp['department'] for exp in profile.get('experiences', [])]
                }
                profile_summaries.append(summary)
            
            # Generate AI ranking prompt
            ranking_prompt = self._create_ranking_prompt(profile_summaries, search_query)
...
"""
    ...
    def _parse_ranking_response(self, response: str) -> List[str]:
        ...
        try:
            ...
            return list(dict.fromkeys(profile_ids))  # Remove duplicates while preserving order
    ...
    def _paginate_results(self, profiles: List[Dict], page: int, limit: int) -> List[Dict]:
    ...
    def _format_public_profile(self, profile: Dict) -> Dict:
        ...
        return {
            ...
            'experience_years': sum(exp.get('duration', 0) for exp in profile.get('experiences', [])),
            'departments': list(set(exp.get('department', '') for exp in profile.get('experiences', []))),
