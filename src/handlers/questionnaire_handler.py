...
from src.utils.auth_utils import verify_jwt_token, get_user_from_token
from src.utils.rbac import require_role, Permission
...
class QuestionnaireHandler:
    ...
    async def generate_questionnaire(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if previous_questionnaires:
                ...
                latest_questionnaire = max(previous_questionnaires, key=lambda q: q.created_at)
                if hasattr(latest_questionnaire, 'responses') and latest_questionnaire.responses:
            ...
            logger.info(f"Generated questionnaire {questionnaire.questionnaire_id} for user {user_id}")
    ...
    async def submit_questionnaire(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not questionnaire_id or not responses:
                return {
                    'statusCode': 400,
                    'body': json.dumps({'error': 'Missing questionnaire_id or responses'})
                }
            ...
            logger.info(f"Submitted questionnaire {questionnaire_id} for user {user_id}")
    ...
    async def get_questionnaire_history(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        """
    ...
    async def regenerate_questionnaire(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Verify questionnaire belongs to user
            original_questionnaire = await self.questionnaire_repo.get_by_id(questionnaire_id)
            ...
            previous_responses = original_questionnaire.responses if original_questionnaire.responses else []
            ...
            logger.info(f"Regenerated questionnaire {new_questionnaire.questionnaire_id} for user {user_id}")
    ...
    async def _update_profile_from_responses(self, user_id: str, responses: list) -> None:
        """
        ...
        try:
            ...
            if update_data:
                ...
                logger.info(f"Updated profile for user {user_id} based on questionnaire responses")
