...
from ..services.matching_engine import get_matching_engine, MatchingCriteria
from ..services.recommendation_service import get_recommendation_service, RecommendationFeedback
from ..utils.auth_utils import verify_jwt_token, get_user_from_token
...
def handle_get_recommendations(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if user_id != requesting_user_id and not require_permission(user_data, Permission.READ_ALL_PROFILES):
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'Cannot access other user recommendations'})
            }
        ...
        if status:
            recommendations = matching_engine.recommendation_repo.get_user_recommendations_by_status(
                user_id, status, limit
            )
        else:
            recommendations = matching_engine.recommendation_repo.get_user_recommendations(
                user_id, limit
            )
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to get recommendations', 'message': str(e)})
        }


def handle_generate_recommendations(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to generate recommendations', 'message': str(e)})
        }


def handle_refresh_recommendations(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not user_id or not opportunity_id:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'user_id and opportunity_id are required'})
            }
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to get match analysis', 'message': str(e)})
        }
...
def handle_batch_generate_recommendations(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not require_permission(user_data, Permission.ADMIN_ACCESS):
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'Admin access required for batch operations'})
            }
        ...
        if 'criteria' in body:
            ...
            criteria = MatchingCriteria(
                ...
                max_recommendations_per_user=criteria_data.get('max_recommendations', 10),
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to generate batch recommendations', 'message': str(e)})
        }


def handle_recommendation_feedback(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if user_id != requesting_user_id and not require_permission(user_data, Permission.READ_ALL_PROFILES):
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'Cannot provide feedback for other user recommendations'})
            }
        ...
        if not recommendation_id or not feedback_type:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'recommendation_id and feedback_type are required'})
            }
        ...
        if feedback_type not in valid_feedback_types:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': f'feedback_type must be one of: {", ".join(valid_feedback_types)}'})
            }
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to record feedback', 'message': str(e)})
        }


def handle_get_recommendation_statistics(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': 'Failed to get statistics', 'message': str(e)})
        }


def handle_get_recommendation_history(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            history = loop.run_until_complete(
                recommendation_service.get_recommendation_history(user_id, limit, status_filter)
            )
        finally:
            loop.close()
...
def handle_get_recommendation_insights(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
