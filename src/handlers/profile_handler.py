...
from src.utils.rbac import require_permission, require_role, Permission, check_resource_access
from src.utils.security_audit import security_auditor, extract_request_info
...
from src.utils.rbac import rbac_manager, Permission, AccessContext
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(path_parts) >= 2:
            ...
            action = path_parameters.get('proxy', '').split('/')[0] if path_parameters.get('proxy') else ''
...
def create_profile(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if profile_repo.profile_exists(user_id):
            return create_response(409, {'error': 'Profile already exists for this user'})
        ...
        profile = VeteranProfile(
            ...
            privacy_settings=body.get('privacy_settings', {'is_publicly_visible': False, 'external_contact': False})
        )
...
def list_profiles(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if user_role == 'admin':
            if not rbac_manager.has_permission(user_role, Permission.VIEW_ANY_PROFILE, access_context):
                return create_response(403, {'error': 'Insufficient permissions'})
        elif user_role == 'external_recruiter':
            if not rbac_manager.has_permission(user_role, Permission.VIEW_PUBLIC_PROFILES, access_context):
                return create_response(403, {'error': 'Insufficient permissions'})
        else:
            return create_response(403, {'error': 'Insufficient permissions'})
        ...
        skills_filter = query_params.get('skills', '').split(',') if query_params.get('skills') else []
        ...
        if user_role == 'admin':
            ...
            if skills_filter:
                ...
                profiles = profile_repo.get_profiles_by_department(department_filter, limit)
            else:
...
@require_permission(Permission.EDIT_OWN_PROFILE, resource_owner_field='profile_user_id')
def generate_business_title(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    """
...
def get_privacy_status(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if status_result['success']:
            return create_response(200, {
                'privacy_status': status_result
            })
        else:
            return create_response(404, {
                'error': status_result.get('error', 'Privacy status not found')
            })
...
@require_permission(Permission.EDIT_OWN_PROFILE, resource_owner_field='profile_user_id')
def update_privacy_settings(event: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        update_result = privacy_manager.update_privacy_settings(profile_user_id, privacy_settings)
