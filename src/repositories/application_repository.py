...
class ApplicationRepository(BaseRepository):
    ...
    def get_applications_for_opportunity(
        ...
        try:
            ...
            logger.error(
                f"Error getting applications for opportunity {opportunity_id}: {e}"
            )
            raise
    ...
    def get_user_applications_by_status(
    ...
    def check_existing_application(
        ...
        try:
            ...
            logger.error(
                f"Error checking existing application for user {user_id}, opportunity {opportunity_id}: {e}"
            )
            raise
    ...
    def get_application_statistics(self) -> dict:
        ...
        try:
            ...
            stats = {"total": len(all_applications), "by_status": {}, "by_type": {}}
            ...
            for item in all_applications:
                ...
                stats["by_status"][app.status] = (
                    stats["by_status"].get(app.status, 0) + 1
                )

                # Count by type
                stats["by_type"][app.application_type] = (
                    stats["by_type"].get(app.application_type, 0) + 1
                )
