...
class QuestionnaireRepository(BaseRepository):
    ...
    def create_questionnaire(self, questionnaire: Questionnaire) -> bool:
        ...
        try:
            ...
            if errors:
                raise ValueError(
                    f"Questionnaire validation failed: {', '.join(errors)}"
                )
    ...
    def get_user_questionnaires(
        ...
        try:
            ...
            if active_only:
                questionnaires = [
                    q for q in questionnaires if q.is_active and not q.is_expired()
                ]
...
class QuestionnaireResponseRepository(BaseRepository):
    ...
    def get_response(
        ...
        try:
            item = self.get_item({"user_id": user_id, "response_id": response_id})
            if item:
                return QuestionnaireResponse.from_dynamodb_item(item)
            return None
        except Exception as e:
            logger.error(
                f"Error getting response {response_id} for user {user_id}: {e}"
            )
            raise
    ...
    def get_responses_for_questionnaire(
        ...
        try:
            ...
            logger.error(
                f"Error getting responses for questionnaire {questionnaire_id}: {e}"
            )
            raise
    ...
    def add_answer(
        self, user_id: str, response_id: str, question_id: str, answer: str
    ) -> bool:
