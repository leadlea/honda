...
class RecommendationRepository(BaseRepository):
    ...
    def create_recommendation(self, recommendation: Recommendation) -> bool:
        ...
        try:
            ...
            if errors:
                raise ValueError(
                    f"Recommendation validation failed: {', '.join(errors)}"
                )
    ...
    def get_recommendation(
        ...
        try:
            ...
            logger.error(
                f"Error getting recommendation {recommendation_id} for user {user_id}: {e}"
            )
            raise
    ...
    def delete_recommendation(self, user_id: str, recommendation_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting recommendation {recommendation_id} for user {user_id}: {e}"
            )
            raise
    ...
    def get_user_recommendations_by_status(
        self, user_id: str, status: str, limit: Optional[int] = None
    ) -> List[Recommendation]:
        ...
        try:
            ...
            logger.error(
                f"Error getting {status} recommendations for user {user_id}: {e}"
            )
            raise

    def get_unviewed_recommendations(
        self, user_id: str, limit: Optional[int] = None
    ) -> List[Recommendation]:
        """Get unviewed recommendations for a user"""
        return self.get_user_recommendations_by_status(user_id, "generated", limit)

    def get_viewed_recommendations(
        self, user_id: str, limit: Optional[int] = None
    ) -> List[Recommendation]:
    ...
    def mark_recommendation_viewed(self, user_id: str, recommendation_id: str) -> bool:
        ...
        try:
            ...
            if not recommendation:
                raise ValueError(
                    f"Recommendation {recommendation_id} not found for user {user_id}"
                )
            ...
            logger.error(
                f"Error marking recommendation {recommendation_id} as viewed: {e}"
            )
            raise
    ...
    def mark_recommendation_applied(self, user_id: str, recommendation_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error marking recommendation {recommendation_id} as applied: {e}"
            )
            raise
    ...
    def mark_recommendation_dismissed(
        ...
        try:
            ...
            logger.error(
                f"Error marking recommendation {recommendation_id} as dismissed: {e}"
            )
            raise

    def get_recommendations_for_opportunity(
        self, opportunity_id: str, limit: Optional[int] = None
    ) -> List[Recommendation]:
        ...
        try:
            ...
            logger.error(
                f"Error getting recommendations for opportunity {opportunity_id}: {e}"
            )
            raise

    def get_high_score_recommendations(
        self, user_id: str, min_score: float = 0.7, limit: Optional[int] = None
    ) -> List[Recommendation]:
        ...
        try:
            ...
            logger.error(
                f"Error getting high score recommendations for user {user_id}: {e}"
            )
            raise
    ...
    def delete_old_recommendations(self, days_old: int = 90) -> int:
        ...
        try:
            ...
            for item in items:
                recommendation = Recommendation.from_dynamodb_item(item)
                self.delete_recommendation(
                    recommendation.user_id, recommendation.recommendation_id
                )
                count += 1
