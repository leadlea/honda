...
from ..models.public_profile import PublicProfile, ContactRequest
...
class PublicProfileRepository(BaseRepository):
    ...
    def create_profile(self, profile: PublicProfile) -> bool:
        ...
        try:
            ...
            if errors:
                raise ValueError(f"Public profile validation failed: {', '.join(errors)}")
    ...
    def search_profiles_by_skills(self, skills: List[str], limit: Optional[int] = None) -> List[PublicProfile]:
    ...
    def search_profiles_by_location(self, location: str, limit: Optional[int] = None) -> List[PublicProfile]:
    ...
    def get_profiles_by_availability(self, availability: str, limit: Optional[int] = None) -> List[PublicProfile]:
    ...
    def get_profiles_by_experience_range(self, min_years: int, max_years: int, limit: Optional[int] = None) -> List[PublicProfile]:
        ...
        try:
            ...
            logger.error(f"Error getting profiles by experience range {min_years}-{max_years}: {e}")
            raise
        ...
        try:
            ...
            if 'skills' in filters and filters['skills']:
                filtered_profiles = [
                    ...
                    if any(
                        skill.lower() in [s.get('name', '').lower() for s in p.get('skills', [])]
            ...
            if 'experience_level' in filters:
                level_map = {'junior': (0, 3), 'mid': (3, 8), 'senior': (8, 15), 'expert': (15, 100)}
            ...
            if 'department' in filters:
                filtered_profiles = [
                    p for p in filtered_profiles
                    if any(
                        filters['department'].lower() in exp.get('department', '').lower()
    ...
    def get_available_categories(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                'availability_options': sorted(list(categories['availability_options'])),
...
class ContactRequestRepository(BaseRepository):
    ...
    def create_request(self, request: ContactRequest) -> bool:
        ...
        try:
            ...
            if errors:
                raise ValueError(f"Contact request validation failed: {', '.join(errors)}")
    ...
    def get_requests_for_profile(self, profile_id: str, limit: Optional[int] = None) -> List[ContactRequest]:
        ...
        try:
            ...
            logger.error(f"Error getting contact requests for profile {profile_id}: {e}")
            raise
    ...
    def get_requests_by_status(self, status: str, limit: Optional[int] = None) -> List[ContactRequest]:
    ...
    def process_request(self, request_id: str, status: str, processor_id: str, notes: str = "") -> bool:
    ...
    def get_requests_by_company(self, company: str, limit: Optional[int] = None) -> List[ContactRequest]:
    ...
    def get_recent_requests(self, days: int = 30, limit: Optional[int] = None) -> List[ContactRequest]:
