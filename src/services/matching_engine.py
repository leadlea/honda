...
class MatchingEngine:
    ...
    async def analyze_match(
        self, veteran_profile: VeteranProfile, opportunity: Opportunity
    ) -> MatchResult:
        """
        ...
        try:
            ...
            match_analysis = await self.ai_service.match_opportunity(
                veteran_profile=veteran_data, opportunity_details=opportunity_data
            )
                    ...
                    match_reasons.append(
                        {
                            ...
                            "weight": skills_data.get("score", 0)
                            * 0.4,  # 40% weight for skills
                            "details": {
                                "matching_skills": skills_data.get(
                                    "matching_skills", []
                                ),
                                "missing_skills": skills_data.get("missing_skills", []),
                                "transferable_skills": skills_data.get(
                                    "transferable_skills", []
                                ),
                            },
                        }
                    ...
                    match_reasons.append(
                        {
                            ...
                            "weight": exp_data.get("score", 0)
                            * 0.3,  # 30% weight for experience
                            "details": {
                                "relevant_experience": exp_data.get(
                                    "relevant_experience", []
                                ),
                    ...
                    match_reasons.append(
                        {
                            ...
                            "weight": career_data.get("score", 0)
                            * 0.2,  # 20% weight for career fit
                            "details": {
                                "alignment_factors": career_data.get(
                                    "alignment_factors", []
                                ),
                                "potential_concerns": career_data.get(
                                    "potential_concerns", []
                                ),
                            },
                        }
                    ...
                    match_reasons.append(
                        {
                            ...
                            "weight": growth_data.get("score", 0)
                            * 0.1,  # 10% weight for growth
                            "details": {
                                "development_opportunities": growth_data.get(
            ...
            return MatchResult(
                ...
                match_summary=match_analysis.get(
            ...
            logger.error(
                f"Error analyzing match for veteran {veteran_profile.user_id} and opportunity {opportunity.opportunity_id}: {e}"
            )
    ...
    async def generate_recommendations_for_veteran(
        ...
        try:
            ...
            for opportunity in opportunities:
                try:
                    match_result = await self.analyze_match(
                        veteran_profile, opportunity
                    )
                    match_results.append(match_result)
                except Exception as e:
                    logger.warning(
                        f"Failed to analyze match for opportunity {opportunity.opportunity_id}: {e}"
                    )
                    continue
            ...
            qualified_matches = [
                match for match in match_results if match.overall_score >= min_threshold
            ]
            ...
            max_recommendations = (
                criteria.max_recommendations_per_user if criteria else 10
            )
    ...
    async def _get_filtered_opportunities(
        self, criteria: Optional[MatchingCriteria] = None
    ) -> List[Opportunity]:
        """
        ...
        try:
            ...
            for opportunity in opportunities:
                ...
                if (
                    criteria.opportunity_types
                    and opportunity.type not in criteria.opportunity_types
                ):
                    continue
    ...
    async def batch_generate_recommendations(
        self, user_ids: List[str], criteria: Optional[MatchingCriteria] = None
    ...
    async def save_recommendations(self, recommendations: List[Recommendation]) -> bool:
        ...
        try:
            ...
            return self.recommendation_repo.batch_create_recommendations(
                recommendations
            )
    ...
    def calculate_match_score_breakdown(
        self, match_reasons: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """
    ...
    async def get_match_explanation(
        ...
        try:
            ...
            if not recommendation:
                raise ValueError(
                    f"Recommendation not found for user {user_id} and opportunity {opportunity_id}"
                )
            ...
            score_breakdown = self.calculate_match_score_breakdown(
            ...
            return {
                ...
                "veteran_profile": {
                    "business_title": veteran_profile.business_title,
                    "key_skills": [
                        skill.get("name") for skill in veteran_profile.skills[:5]
                    ],
                    "experience_years": sum(
                        exp.get("duration", 0) for exp in veteran_profile.experiences
                    ),
                },
                "opportunity": {
                    "title": opportunity.title,
