...
from .matching_engine import get_matching_engine, MatchingCriteria
...
class RecommendationService:
    ...
    async def generate_personalized_recommendations(
        ...
        try:
            ...
            if prevent_duplicates:
                existing_opportunity_ids = await self._get_recent_recommendation_opportunity_ids(
                    user_id, self.duplicate_prevention_days
                )
            
            # Generate new recommendations using matching engine
            new_recommendations = await self.matching_engine.generate_recommendations_for_veteran(
                user_id, criteria
            )
            ...
            for rec in new_recommendations:
                if rec.opportunity_id not in existing_opportunity_ids:
                    ...
                    logger.info(f"Filtered duplicate recommendation for opportunity {rec.opportunity_id}")
            ...
            if len(personalized_recommendations) > self.max_recommendations_per_batch:
                personalized_recommendations = personalized_recommendations[:self.max_recommendations_per_batch]
            
            logger.info(f"Generated {len(personalized_recommendations)} personalized recommendations for user {user_id}")
    ...
    async def _get_recent_recommendation_opportunity_ids(
        ...
        try:
            ...
            all_recommendations = self.recommendation_repo.get_user_recommendations(user_id)
    ...
    async def _get_user_feedback_patterns(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            user_recommendations = self.recommendation_repo.get_user_recommendations(user_id, limit=100)
            ...
            for rec in user_recommendations:
                ...
                if rec.status == "applied":
                    ...
                    patterns["preferred_opportunity_types"][opportunity.type] += 2  # Higher weight
            ...
            if applied_scores:
                patterns["average_applied_score"] = sum(applied_scores) / len(applied_scores)
            if dismissed_scores:
                patterns["average_dismissed_score"] = sum(dismissed_scores) / len(dismissed_scores)
    ...
    def _calculate_personalization_factor(
        ...
        try:
            # Get opportunity details
            opportunity = self.opportunity_repo.get_opportunity(recommendation.opportunity_id)
            if not opportunity:
                return 1.0
    ...
    async def _update_learning_model(self, user_id: str) -> None:
        ...
        try:
            ...
            user_recommendations = self.recommendation_repo.get_user_recommendations(user_id, limit=50)
            
            feedback_count = sum(
                1 for rec in user_recommendations 
                if any(reason.get("category") == "user_feedback" for reason in rec.match_reasons)
            )
            
            if feedback_count >= self.min_feedback_for_learning:
                logger.info(f"Updating learning model for user {user_id} with {feedback_count} feedback items")
    ...
    async def get_recommendation_statistics(
        ...
        try:
            ...
            viewed_count = sum(1 for rec in recommendations if rec.status in ["viewed", "applied", "dismissed"])
            applied_count = sum(1 for rec in recommendations if rec.status == "applied")
            dismissed_count = sum(1 for rec in recommendations if rec.status == "dismissed")
    ...
    async def get_recommendation_history(
        ...
        try:
            # Get recommendations
            if status_filter:
                recommendations = self.recommendation_repo.get_user_recommendations_by_status(
                    user_id, status_filter, limit
                )
            else:
                recommendations = self.recommendation_repo.get_user_recommendations(user_id, limit)
    ...
    async def refresh_recommendations_with_feedback_learning(
        ...
        try:
            ...
            logger.error(f"Error refreshing recommendations with feedback learning: {e}")
            raise
            ...
            insights = {
                ...
                "preferences": {
                    "top_opportunity_types": dict(
                        sorted(
                            feedback_patterns.get("preferred_opportunity_types", {}).items(),
                            key=lambda x: x[1],
                            reverse=True
                        )[:5]
                    ),
