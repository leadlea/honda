...
class PrivacyManager:
    ...
    def update_privacy_settings(self, user_id: str, privacy_settings: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _sync_external_visibility(self, profile: VeteranProfile, was_public: bool) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error syncing external visibility for user {profile.user_id}: {e}")
            return {
    ...
    def _create_public_profile(self, profile: VeteranProfile) -> Dict[str, Any]:
        ...
        try:
            ...
            skills_list = [skill.get('name', '') for skill in profile.skills if skill.get('name')]
            
            # Calculate total experience years
            total_experience = sum(exp.get('duration', 0) for exp in profile.experiences) // 12  # Convert months to years
            ...
            public_profile = PublicProfile(
                ...
                contact_preferences={
                    'allow_contact': profile.privacy_settings.get('external_contact', False),
            ...
            if success:
                logger.info(f"Public profile created/updated for user {profile.user_id}")
                return {
                    'success': True,
                ...
                return {
                    'success': False,
                    'details': 'Failed to create public profile'
                }
                
        except Exception as e:
            logger.error(f"Error creating public profile for user {profile.user_id}: {e}")
            return {
                'success': False,
    ...
    def _update_public_profile(self, profile: VeteranProfile) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_public:
                ...
                skills_list = [skill.get('name', '') for skill in profile.skills if skill.get('name')]
                total_experience = sum(exp.get('duration', 0) for exp in profile.experiences) // 12
            ...
            logger.error(f"Error updating public profile for user {profile.user_id}: {e}")
            return {
                'success': False,
    ...
    def _remove_public_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                return {
                    'success': False,
                    'details': 'Failed to remove public profile'
                }
    ...
    def get_privacy_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'is_publicly_visible': profile.privacy_settings.get('is_publicly_visible', False),
                'external_contact_allowed': profile.privacy_settings.get('external_contact', False),
                'public_profile_exists': public_profile is not None,
                'last_updated': profile.last_updated,
                'public_profile_last_updated': public_profile.updated_at if public_profile else None
            }
    ...
    def bulk_privacy_update(self, privacy_updates: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        results['success_rate'] = len(results['successful_updates']) / results['total_processed'] if results['total_processed'] > 0 else 0
        
        logger.info(f"Bulk privacy update completed: {len(results['successful_updates'])}/{results['total_processed']} successful")
        
        return results
    
    def validate_privacy_settings(self, privacy_settings: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if privacy_settings.get('external_contact', False) and not privacy_settings.get('is_publicly_visible', False):
            warnings.append("external_contact is enabled but profile is not publicly visible")
