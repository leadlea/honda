...
from src.utils.performance import (
    ConnectionPool, BedrockOptimizer, performance_timer, bedrock_optimizer
)
...
class BedrockClient:
    ...
    def _initialize_clients(self) -> None:
        ...
        for region in regions:
            try:
                ...
                logger.warning(f"Failed to initialize client for region {region.value}: {e}")
    ...
    async def invoke_claude(
        ...
        region_override: Optional[BedrockRegion] = None
    ) -> BedrockResponse:
        """
        ...
        """
        regions_to_try = [region_override] if region_override else [self.primary_region] + self.fallback_regions
        
        # Check cache first
        prompt_hash = hashlib.md5(request.prompt.encode()).hexdigest()
        cached_response = self.optimizer.get_cached_response(prompt_hash, self.model.value)
        if cached_response:
            return BedrockResponse(
                success=True,
        ...
        for region in regions_to_try:
            ...
            for attempt in range(self.max_retries + 1):
                ...
                try:
                    logger.debug(f"Invoking Claude in {region.value}, attempt {attempt + 1}")
                    ...
                    if result.success and result.content:
                        self.optimizer.cache_response(prompt_hash, self.model.value, result.content)
                    ...
                    last_error = e
                    logger.warning(f"Claude invocation failed in {region.value}, attempt {attempt + 1}: {e}")
