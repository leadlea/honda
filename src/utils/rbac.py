...
class RBACManager:
    ...
    def has_permission(self, user_role: str, permission: Permission, 
    ...
    def _apply_context_rules(self, role: Role, permission: Permission, 
                           context: Optional[AccessContext]) -> bool:
        """
        ...
        if role != Role.ADMIN and context.resource_owner_id:
            if permission in [Permission.VIEW_OWN_PROFILE, Permission.EDIT_OWN_PROFILE,
                            Permission.VIEW_OWN_APPLICATIONS, Permission.VIEW_QUESTIONNAIRE_RESULTS]:
        ...
        if role == Role.EXTERNAL_RECRUITER:
            if permission in [Permission.VIEW_PUBLIC_PROFILES, Permission.SEARCH_PUBLIC_PROFILES]:
    ...
    def can_access_resource(self, user_role: str, user_id: str, 
                          resource_type: str, resource_owner_id: str,
    ...
    def decorator(func):
        ...
        def wrapper(event, context):
            ...
            if user_role not in allowed_roles:
                # Log security event
                log_access_denied(user_id, user_role, f"role_check:{','.join(allowed_roles)}")
...
def check_resource_access(user_role: str, user_id: str, resource_type: str,
                         resource_owner_id: str, action: str, **kwargs) -> bool:
    """
    Check if user can access a specific resource.
...
def log_access_granted(user_id: str, user_role: str, permission: str, 
...
def log_access_denied(user_id: str, user_role: str, permission: str, 
                     context: AccessContext = None):
    """Log access denial."""
    log_entry = {
        'timestamp': datetime.now(timezone.utc).isoformat(),
