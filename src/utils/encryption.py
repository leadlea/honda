    ...
    def _get_fernet_key(self) -> Fernet:
        ...
        if self._fernet_key is None:
            try:
                ...
                password = os.environ.get('ENCRYPTION_PASSWORD', 'default-dev-key').encode()
...
class PIIProtectionService:
    ...
    SENSITIVE_FIELDS = {
        'salary', 'performance_rating', 'medical_info', 'emergency_contact'
    }
    ...
    def protect_pii_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field, value in data.items():
            if self._is_pii_field(field) and value:
                try:
                    protected_data[field] = self.encryption_service.encrypt_data(str(value))
        ...
        for field, value in data.items():
            if field.endswith('_encrypted') and value:
                ...
                if original_field in data:
                    try:
                        unprotected_data[original_field] = self.encryption_service.decrypt_data(
                            data[original_field]
                        )
                        del unprotected_data[field]  # Remove encryption flag
                    except Exception as e:
                        logger.error(f"Failed to decrypt PII field {original_field}: {e}")
    ...
    def _is_sensitive_field(self, field_name: str) -> bool:
        ...
        field_lower = field_name.lower()
        return any(sensitive_field in field_lower for sensitive_field in self.SENSITIVE_FIELDS)
