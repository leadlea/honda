...
class TestApplicationHandler:
    ...
    def test_submit_application_success(self, mock_extract_user):
        ...
        self.handler.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
        ...
        self.handler.application_repo.check_existing_application.assert_called_once_with('user123', 'opp123')
    ...
    def test_submit_application_inactive_opportunity(self, mock_extract_user):
        ...
        self.handler.opportunity_repo.get_opportunity.return_value = inactive_opportunity
    ...
    def test_submit_application_duplicate(self, mock_extract_user):
        ...
        mock_extract_user.return_value = {'user_id': 'user123', 'role': 'veteran'}
        self.handler.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
        self.handler.application_repo.check_existing_application.return_value = self.sample_application
    ...
    def test_get_user_applications_success(self, mock_extract_user):
        ...
        # Setup
        mock_extract_user.return_value = {'user_id': 'user123', 'role': 'veteran'}
        self.handler.application_repo.get_user_applications.return_value = [self.sample_application]
        self.handler.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
    ...
    def test_get_user_applications_with_status_filter(self, mock_extract_user):
        ...
        # Setup
        mock_extract_user.return_value = {'user_id': 'user123', 'role': 'veteran'}
        self.handler.application_repo.get_user_applications_by_status.return_value = [self.sample_application]
        self.handler.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
        ...
        # Assert
        assert result['statusCode'] == 200
        self.handler.application_repo.get_user_applications_by_status.assert_called_once_with('user123', 'submitted', 10)
    ...
    def test_get_application_details_success(self, mock_extract_user):
        ...
        # Setup
        mock_extract_user.return_value = {'user_id': 'user123', 'role': 'veteran'}
        self.handler.application_repo.get_application.return_value = self.sample_application
        self.handler.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
    ...
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_get_application_details_access_denied(self, mock_extract_user, mock_rbac_manager):
    ...
    def test_withdraw_application_success(self, mock_extract_user):
        ...
        self.handler.application_repo.withdraw_application.assert_called_once_with('app123', 'user123')
    ...
    def test_withdraw_application_failure(self, mock_extract_user):
        ...
        # Setup
        mock_extract_user.return_value = {'user_id': 'user123', 'role': 'veteran'}
        self.handler.application_repo.withdraw_application.side_effect = ValueError("Cannot withdraw")
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_update_application_status_success(self, mock_extract_user, mock_rbac_manager):
        ...
        event = {
            ...
            'body': json.dumps({'status': 'under_review', 'notes': 'Reviewing application'})
        }
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_update_application_status_insufficient_permissions(self, mock_extract_user, mock_rbac_manager):
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_update_application_status_missing_status(self, mock_extract_user, mock_rbac_manager):
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_get_applications_for_opportunity_success(self, mock_extract_user, mock_rbac_manager):
        """Test getting applications for opportunity successfully"""
        # Setup
        mock_extract_user.return_value = {'user_id': 'admin123', 'role': 'admin'}
        mock_rbac_manager.has_permission.return_value = True
        self.handler.application_repo.get_applications_for_opportunity.return_value = [self.sample_application]
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_get_applications_for_opportunity_insufficient_permissions(self, mock_extract_user, mock_rbac_manager):
    ...
    def test_notify_stakeholders(self):
        ...
        self.handler._notify_stakeholders(self.sample_application, self.sample_opportunity, 'application_submitted')
    ...
    def test_get_application_history_success(self, mock_extract_user):
        ...
        with patch('src.handlers.application_handler.ApplicationStatusService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.get_application_history.return_value = {
                'success': True,
                'application': self.sample_application.to_dynamodb_item(),
                'history': [{'event_type': 'application_submitted', 'timestamp': '2024-01-01T00:00:00'}]
            }
    ...
    def test_get_application_communications_success(self, mock_extract_user):
        ...
        with patch('src.handlers.application_handler.ApplicationStatusService') as mock_service_class:
            ...
            # Assert
            assert result['statusCode'] == 200
            mock_service.get_application_communications.assert_called_once_with('app123', 'user123')
    
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_update_application_status_with_workflow_success(self, mock_extract_user, mock_rbac_manager):
    ...
    @patch('src.handlers.application_handler.rbac_manager')
    @patch('src.handlers.application_handler.extract_user_from_event')
    def test_update_application_status_with_workflow_invalid_transition(self, mock_extract_user, mock_rbac_manager):
