...
from src.services.application_status_service import ApplicationStatusService, StatusUpdate, CommunicationMessage
...
class TestApplicationStatusService:
    ...
    def test_update_status_with_workflow_success(self):
        ...
        # Setup
        self.service.application_repo.get_application.return_value = self.sample_application
    ...
    def test_get_application_history_success(self):
        ...
        # Setup
        self.service.application_repo.get_application.return_value = self.sample_application
        self.service.opportunity_repo.get_opportunity.return_value = self.sample_opportunity
        ...
        submission_event = next(
            (event for event in result['history'] if event['event_type'] == 'application_submitted'), 
            None
        )
    ...
    def test_send_communication_message_success(self):
        ...
        result = self.service.send_communication_message(
            "app123", "user123", "Hello, I have a question about this position", "general"
        )
    ...
    def test_is_valid_status_transition(self):
        ...
        # Valid transitions
        assert self.service._is_valid_status_transition('submitted', 'under_review') is True
        assert self.service._is_valid_status_transition('submitted', 'rejected') is True
        assert self.service._is_valid_status_transition('under_review', 'accepted') is True
        assert self.service._is_valid_status_transition('under_review', 'interview_scheduled') is True
        
        # Invalid transitions
        assert self.service._is_valid_status_transition('submitted', 'accepted') is False
        assert self.service._is_valid_status_transition('accepted', 'under_review') is False
        assert self.service._is_valid_status_transition('rejected', 'accepted') is False
        assert self.service._is_valid_status_transition('withdrawn', 'under_review') is False
    ...
    def test_notify_status_change(self, mock_logger):
        ...
        self.service._notify_status_change(
            self.sample_application, "submitted", "under_review", "admin123", "Reviewing"
        )
