...
class TestRecommendationService:
    ...
    async def test_generate_personalized_recommendations_no_duplicates(
        ...
        recommendation_service.matching_engine.generate_recommendations_for_veteran = (
            AsyncMock(return_value=sample_recommendations)
        )
        ...
        result = await recommendation_service.generate_personalized_recommendations(
            "user123"
        )
    ...
    @pytest.mark.asyncio
    async def test_apply_personalization(
        self, recommendation_service, sample_recommendations
    ):
        ...
        recommendation_service._get_user_feedback_patterns = AsyncMock(
            return_value=feedback_patterns
        )
        ...
        recommendation_service.opportunity_repo.get_opportunity.return_value = (
            opportunity
        )

        # Apply personalization
        result = await recommendation_service._apply_personalization(
            "user123", sample_recommendations[:1]
        )
    ...
    @pytest.mark.asyncio
    async def test_record_recommendation_feedback(
        self, recommendation_service, sample_opportunity
    ):
        ...
        recommendation_service.recommendation_repo.get_recommendation.return_value = (
            recommendation
        )
        recommendation_service.recommendation_repo.update_recommendation.return_value = (
            True
        )
    ...
    async def test_record_feedback_applied(self, recommendation_service):
        ...
        feedback = RecommendationFeedback(
            recommendation_id="rec1", user_id="user123", feedback_type="applied"
        )
    ...
    async def test_get_recommendation_statistics(self, recommendation_service):
        ...
        recommendations[0].match_reasons.append(
            {"category": "user_feedback", "details": {"feedback_score": 5.0}}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_recommendation_history(
        self, recommendation_service, sample_recommendations
    ):
    ...
    @pytest.mark.asyncio
    async def test_refresh_recommendations_with_feedback_learning(
        ...
        recommendation_service.matching_engine.save_recommendations = AsyncMock(
            return_value=True
        )

        # Refresh recommendations
        result = (
            await recommendation_service.refresh_recommendations_with_feedback_learning(
                "user123"
            )
        )
    ...
    async def test_get_recommendation_insights(self, recommendation_service):
        ...
        recommendation_service.get_recommendation_statistics = AsyncMock(
            return_value=stats
        )
        ...
        # Check preferences
        assert (
            insights["preferences"]["top_opportunity_types"]["internal_transfer"] == 5
        )
...
class TestRecommendationFeedback:
    ...
    def test_feedback_auto_timestamp(self):
        ...
        feedback = RecommendationFeedback(
            recommendation_id="rec123", user_id="user123", feedback_type="negative"
        )
