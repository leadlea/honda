...
class TestContactHandler:
    ...
    def test_submit_contact_request_success(self, mock_log, mock_profile_repo_class, mock_contact_repo_class):
    ...
    @patch('src.handlers.contact_handler.ContactRequestRepository')
    @patch('src.handlers.contact_handler.PublicProfileRepository')
    def test_submit_contact_request_rate_limited(self, mock_profile_repo_class, mock_contact_repo_class):
    ...
    @patch('src.handlers.contact_handler.ContactRequestRepository')
    @patch('src.handlers.contact_handler.PublicProfileRepository')
    def test_get_contact_requests_success(self, mock_profile_repo_class, mock_contact_repo_class):
        """Test successful retrieval of contact requests"""
        # Setup
        mock_profile_repo = Mock()
        mock_profile_repo_class.return_value = mock_profile_repo
        mock_profile_repo.get_profile_by_user_id.return_value = Mock(profile_id='profile_123')
        
        mock_contact_repo = Mock()
        mock_contact_repo_class.return_value = mock_contact_repo
        mock_contact_repo.get_requests_for_profile.return_value = [self.mock_contact_request]
    ...
    @patch('src.handlers.contact_handler.PublicProfileRepository')
    @patch('src.handlers.contact_handler.audit_security_event')
    def test_process_contact_request_approve(self, mock_log, mock_profile_repo_class, mock_contact_repo_class):
        ...
        event = {
            ...
            'body': json.dumps({
                'action': 'approve',
                'notes': 'Looks like a good opportunity'
            })
        }
    ...
    def test_is_rate_limited(self):
        ...
        self.handler.contact_repo.get_recent_requests = Mock(return_value=recent_requests)
        ...
        assert self.handler._is_rate_limited('other@company.com', 'profile_123') == False
