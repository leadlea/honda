...
from src.utils.rbac import (
    Role, Permission, AccessContext, RBACManager, rbac_manager,
    require_permission, require_role, check_resource_access,
    get_user_permissions, validate_role, get_available_roles
)
...
class TestRBACManager:
    ...
    def test_external_recruiter_permissions(self):
        ...
        assert self.rbac.has_permission('external_recruiter', Permission.VIEW_PUBLIC_PROFILES)
        assert self.rbac.has_permission('external_recruiter', Permission.SEARCH_PUBLIC_PROFILES)
        assert self.rbac.has_permission('external_recruiter', Permission.CONTACT_VETERANS)
        assert self.rbac.has_permission('external_recruiter', Permission.CREATE_OPPORTUNITIES)
        
        # External recruiters should NOT have internal permissions
        assert not self.rbac.has_permission('external_recruiter', Permission.VIEW_OWN_PROFILE)
        assert not self.rbac.has_permission('external_recruiter', Permission.TAKE_QUESTIONNAIRE)
        assert not self.rbac.has_permission('external_recruiter', Permission.CREATE_USERS)
    ...
    def test_context_based_access_other_resource(self):
        ...
        assert not self.rbac.has_permission('veteran', Permission.VIEW_OWN_PROFILE, context)
        assert not self.rbac.has_permission('veteran', Permission.EDIT_OWN_PROFILE, context)
    ...
    def test_external_recruiter_public_profile_access(self):
        ...
        assert self.rbac.has_permission('external_recruiter', Permission.VIEW_PUBLIC_PROFILES, public_context)
        ...
        assert not self.rbac.has_permission('external_recruiter', Permission.VIEW_PUBLIC_PROFILES, private_context)
    ...
    def test_veteran_opportunity_application(self):
        ...
        assert self.rbac.has_permission('veteran', Permission.APPLY_TO_OPPORTUNITIES, active_context)
        ...
        assert not self.rbac.has_permission('veteran', Permission.APPLY_TO_OPPORTUNITIES, inactive_context)
...
class TestRBACDecorators:
    ...
    def test_require_permission_decorator_success(self):
        ...
        with patch('src.utils.rbac.rbac_manager.has_permission', return_value=True), \
    ...
    def test_require_permission_decorator_denied(self):
        ...
        with patch('src.utils.rbac.rbac_manager.has_permission', return_value=False), \
...
class TestRBACUtilities:
    ...
    def test_check_resource_access(self):
        ...
        with patch('src.utils.rbac.rbac_manager.can_access_resource', return_value=True):
