...
from src.services.matching_engine import (
    MatchingEngine, MatchResult, MatchingCriteria, get_matching_engine
)
...
class TestMatchingEngine:
    ...
    def sample_veteran_profile(self):
        ...
        return VeteranProfile(
            ...
            skills=[
                ...
                {"name": "AWS", "level": "Advanced", "years": 5, "certifications": ["AWS Solutions Architect"]},
                {"name": "Machine Learning", "level": "Intermediate", "years": 3, "certifications": []}
            ],
            experiences=[
                {
            ...
            preferences={
                "preferred_roles": ["Technical Lead", "Engineering Manager", "Solutions Architect"],
                "work_style": "hybrid",
                "locations": ["Tokyo", "Remote"]
            },
            questionnaire_responses=[
                {"question": "Leadership experience", "answer": "5+ years managing teams"},
                {"question": "Career goals", "answer": "Transition to technical leadership role"}
            ]
        )
        ...
        return {
            ...
            "match_analysis": {
                ...
                "experience_relevance": {
                    "score": 0.8,
                    "relevant_experience": ["Senior Software Engineer", "Team Leadership"],
                    "experience_gaps": ["Kubernetes management"]
                },
                "career_fit": {
                    "score": 0.85,
                    "alignment_factors": ["Leadership aspirations", "Technical background"],
            ...
            "recommendation": {
                ...
                "success_factors": ["Strong technical background", "Leadership experience"],
    ...
    @pytest.mark.asyncio
    async def test_analyze_match_success(self, matching_engine, sample_veteran_profile, 
                                       sample_opportunity, sample_ai_match_response):
        ...
        result = await matching_engine.analyze_match(sample_veteran_profile, sample_opportunity)
    ...
    @pytest.mark.asyncio
    async def test_analyze_match_ai_failure(self, matching_engine, sample_veteran_profile, 
                                          sample_opportunity):
    ...
    @pytest.mark.asyncio
    async def test_generate_recommendations_for_veteran(self, matching_engine, 
        ...
        matching_engine.opportunity_repo.get_active_opportunities.return_value = [sample_opportunity]
        ...
        recommendations = await matching_engine.generate_recommendations_for_veteran("veteran123")
    ...
    @pytest.mark.asyncio
    async def test_generate_recommendations_with_criteria(self, matching_engine, 
        ...
        # Mock repository responses
        matching_engine.veteran_repo.get_profile.return_value = sample_veteran_profile
        matching_engine.opportunity_repo.get_active_opportunities.return_value = opportunities
        ...
        matching_engine.ai_service.match_opportunity = AsyncMock(side_effect=mock_ai_response)
    ...
    async def test_save_recommendations(self, matching_engine):
        ...
        matching_engine.recommendation_repo.batch_create_recommendations.return_value = True
    ...
    async def test_refresh_recommendations_for_veteran(self, matching_engine):
        ...
        matching_engine.save_recommendations.assert_called_once_with(mock_recommendations)
    ...
    @pytest.mark.asyncio
    async def test_get_match_explanation(self, matching_engine, sample_veteran_profile, 
                                       sample_opportunity):
        ...
        # Mock repository responses
        matching_engine.recommendation_repo.get_recommendation.return_value = recommendation
        matching_engine.veteran_repo.get_profile.return_value = sample_veteran_profile
        matching_engine.opportunity_repo.get_opportunity.return_value = sample_opportunity
        
        # Get match explanation
        explanation = await matching_engine.get_match_explanation("veteran123", "opp123")
        ...
        assert explanation["veteran_profile"]["business_title"] == "Senior Software Engineer"
