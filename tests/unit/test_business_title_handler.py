...
class TestBusinessTitleHandler:
    ...
    @pytest.mark.asyncio
    async def test_generate_business_titles_success(self, mock_get_ai_service, mock_get_user):
        ...
        mock_profile = VeteranProfile(
            ...
            experiences=[
                {'title': 'Software Engineer', 'department': 'Engineering', 'duration': 3, 'achievements': []}
            ],
    ...
    @patch('src.handlers.business_title_handler.get_ai_service')
    @pytest.mark.asyncio
    async def test_regenerate_business_titles_success(self, mock_get_ai_service, mock_get_user):
        ...
        event = {
            ...
            'body': json.dumps({
                'context': {
                    'additional_interests': ['Innovation', 'Mentoring']
                }
            })
        }
    ...
    async def test_store_title_generation_history(self):
        ...
        titles_data = {
            'titles': [
                {'title': 'New Title', 'market_appeal': 'high'}
            ],
            'recommended_title': 'New Title',
            'reasoning': 'Test reasoning'
        }
        
        await self.handler._store_title_generation_history('user123', titles_data, regenerated=True)
