...
class TestProfileHandler:
    ...
    def test_handler_routing_create_profile(self, mock_extract_user):
        ...
        with patch(
            "src.handlers.profile_handler.create_profile"
    ...
    def test_create_profile_success(self, mock_repo_class):
        ...
        event = {"user": self.mock_user, "body": json.dumps(self.mock_profile_data)}

        with patch(
            "src.handlers.profile_handler.security_auditor"
        ) as mock_auditor, patch(
            "src.handlers.profile_handler.extract_request_info"
    ...
    def test_create_profile_validation_error(self, mock_repo_class):
        ...
        invalid_data = {"skills": [{"name": "Python"}]}  # Missing level and years

        event = {"user": self.mock_user, "body": json.dumps(invalid_data)}
    ...
    def test_get_profile_success(self, mock_check_access, mock_repo_class):
        ...
        mock_profile = VeteranProfile(user_id="test-user-123", **self.mock_profile_data)
        mock_repo.get_profile.return_value = mock_profile

        event = {"user": self.mock_user, "path": "/profiles/test-user-123"}
    ...
    def test_get_profile_access_denied(self, mock_check_access, mock_repo_class):
        ...
        event = {"user": self.mock_user, "path": "/profiles/other-user-456"}
    ...
    def test_update_profile_success(self, mock_repo_class):
        ...
        existing_profile = VeteranProfile(
            user_id="test-user-123", **self.mock_profile_data
        )
        ...
        update_data = {
            ...
            "skills": [
                {"name": "Python", "level": "Expert", "years": 6, "certifications": []}
            ],
        }
    ...
    def test_delete_profile_success(self, mock_repo_class):
        ...
        event = {"user": admin_user, "path": "/profiles/test-user-123"}
    ...
    def test_list_profiles_admin(
        self, mock_rbac_manager, mock_user_repo_class, mock_profile_repo_class
    ):
        ...
        mock_profile_repo.scan.return_value = [
            p.to_dynamodb_item() for p in mock_profiles
        ]
        ...
        event = {"user": admin_user, "path": "/profiles", "queryStringParameters": None}
    ...
    @patch("src.handlers.profile_handler.UserRepository")
    @patch("src.handlers.profile_handler.rbac_manager")
    def test_list_profiles_external_recruiter(
        self, mock_rbac_manager, mock_user_repo_class, mock_repo_class
    ):
        ...
        public_profile = VeteranProfile(
            user_id="user1", business_title="Engineer", is_publicly_visible="true"
        )
        mock_repo.get_public_profiles.return_value = [public_profile]
        mock_user_repo.get_user_by_id.return_value = (
            None  # External recruiters don't get user data
        )
        ...
        )  # User data not included for external recruiters
    ...
    def test_update_privacy_settings_success(self, mock_privacy_manager):
        ...
        privacy_data = {"is_publicly_visible": True, "external_contact": False}
        ...
        with patch(
            ...
            mock_privacy_manager.validate_privacy_settings.assert_called_once_with(
                privacy_data
            )
            mock_privacy_manager.update_privacy_settings.assert_called_once_with(
                "test-user-123", privacy_data
            )
            mock_auditor.log_profile_access.assert_called_once()
    ...
    def test_invalid_json_body(self):
        ...
        event = {"user": self.mock_user, "body": "invalid json"}
